...
export default function Designations() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const err: any = error;
      console.error('Error saving designation:', err?.response?.status, err?.response?.data || err?.message);
      setErrorMessage(
        err?.response?.data?.message ||
        err?.response?.data?.error ||
  ...
  const handleDelete = (designation: Designation) => {
    setDeleteTarget({ id: designation.id, name: designation.name });
  ...
  const confirmDelete = async () => {
    ...
    try {
      await axios.delete(`${API_URL}/api/hr/designations/${deleteTarget.id}`, {
        headers: { Authorization: `Bearer ${token}`, Accept: 'application/json' },
      });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-cyan-50 to-teal-50 relative overflow-hidden">
      ...
      <main className="relative z-10 max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 flex justify-between items-center">
          <div>
            <div className="flex items-center space-x-3 mb-4">
              ...
              <div>
                <h2 className="text-3xl font-bold text-gray-900">Designation Management</h2>
                <p className="text-gray-600">Manage job positions and titles</p>
              </div>
            </div>
        ...
        <div className="mt-4 flex flex-col sm:flex-row items-center justify-between">
          <div className="text-sm text-gray-600 mb-3 sm:mb-0">
            Showing <span className="font-medium">{from || 0}</span> to <span className="font-medium">{to || 0}</span> of <span className="font-medium">{total}</span> results
          </div>
          <div className="flex items-center space-x-2">
            <button
              className="px-3 py-2 border border-gray-300 rounded-xl text-sm disabled:opacity-50"
              disabled={page <= 1}
            ...
            {Array.from({ length: Math.min(lastPage, 5) }, (_, i) => {
              // Calculate windowed page numbers around current page
              const delta = 2;
              ...
              return pages[i] ? (
                <button
                  ...
                  className={`px-3 py-2 border rounded-xl text-sm ${pages[i] === page ? 'bg-blue-500 text-white border-blue-500' : 'border-gray-300 text-gray-700 hover:bg-gray-50'}`}
                >
                  {pages[i]}
                </button>
              ) : null;
      ...
      {showConfirmDelete && deleteTarget && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            <div className="bg-gradient-to-r from-red-500 to-pink-500 p-5 rounded-t-2xl">
            ...
            <div className="p-6">
              ...
              <div className="mt-6 flex justify-end space-x-3">
                <button
                  className="px-5 py-2 text-gray-700 border border-gray-300 rounded-xl hover:bg-gray-50"
                  onClick={() => { setShowConfirmDelete(false); setDeleteTarget(null); }}
                >
                  Cancel
                </button>
                <button
                  className="px-5 py-2 bg-gradient-to-r from-red-500 to-pink-500 text-white rounded-xl hover:from-red-600 hover:to-pink-600"
      ...
      {showErrorModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            <div className="bg-gradient-to-r from-yellow-500 to-amber-500 p-5 rounded-t-2xl">
              <h3 className="text-white text-lg font-bold">Action Failed</h3>
              <p className="text-white/80 text-sm">Please review the details below.</p>
            </div>
            <div className="p-6">
              <p className="text-gray-800">{errorMessage || 'An unexpected error occurred.'}</p>
              <div className="mt-4 text-sm text-gray-600">
                {/* Helpful hint for common case */}
                <p>If this is a delete conflict, the designation may be assigned to employees. Remove the assignment before deleting.</p>
              </div>
              <div className="mt-6 flex justify-end">
                <button
                  className="px-5 py-2 bg-gradient-to-r from-blue-500 to-cyan-500 text-white rounded-xl hover:from-blue-600 hover:to-cyan-600"
                  onClick={() => { setShowErrorModal(false); setErrorMessage(null); }}
                >
                  Close
                </button>
              </div>
